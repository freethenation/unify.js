(function() {
  var Box, DICT_FLAG, DictFlag, HIDDEN_VAR_PREFIX, TreeTin, VarTin, Variable, bind, bind_tins, box, boxit, extern, g_hidden_var_counter, get_tin, isHiddenVar, map, str, toJson, types, unboxit, _unify;
  ...
  toJson = function(elem) {
    ...
    } else if (elem instanceof Box || elem instanceof VarTin || elem instanceof TreeTin || elem instanceof Variable || elem instanceof DictFlag) {
      return str(elem);
  ...
  TreeTin = (function() {
    ...
    TreeTin.prototype.toString = function() {
      ...
      */
      return "new TreeTin(" + (toJson(this.node)) + ", " + (toJson(this.varlist)) + ")";
    };

    TreeTin.prototype.get = function(varName) {
      var vartin;
    ...
    TreeTin.prototype.getAll = function() {
      var j, key;
      j = {};
    ...
    TreeTin.prototype.unbox = function() {
      return unboxit(this.node, this.varlist);
    };
  ...
  VarTin = (function() {
    ...
    VarTin.prototype.toString = function() {
      /* Returns the representation of the tin. This is very useful for inspecting the current state of the tin.
      */
      return "new VarTin(" + (toJson(this.name)) + ", " + (toJson(this.node)) + ", " + (toJson(this.varlist)) + ")";
    };

    VarTin.prototype.unbox = function() {
      if (this.node) {
        return unboxit(this.node, this.varlist);
      } else {
  ...
  _unify = function(n1, v1, n2, v2, changes) {
    var idx1, idx2, isListVar, realLength, removeListVars, t1, t2;
    if (changes == null) {
      changes = [];
    }
    ...
    if (n1 instanceof Variable && n2 instanceof Variable) {
      ...
      isListVar = function(n) {
        return n instanceof Variable && n.isListVar;
      };
      realLength = function(n) {
        return n.length - (n.hasListVar ? 1 : 0);
      };
      removeListVars = function(arr, varList) {
        var i, ret, _i, _len;
        ret = [];
        for (_i = 0, _len = arr.length; _i < _len; _i++) {
          i = arr[_i];
      ...
      if (realLength(n1) === realLength(n2)) {
        ...
        while (idx1 < n1.length && idx2 < n2.length) {
          if (isListVar(n1[idx1])) {
            if (!_unify(n1[idx1], v1, [], [], changes)) {
              return false;
            }
            idx1++;
            continue;
          } else if (isListVar(n2[idx2])) {
            if (!_unify(n2[idx2], v2, [], [], changes)) {
              return false;
            }
            idx2++;
            continue;
          } else {
            if (!_unify(n1[idx1], v1, n2[idx2], v2, changes)) {
              return false;
            }
            idx1++;
            idx2++;
      ...
      while (idx2 < n2.length) {
        if (isListVar(n1[idx1])) {
          if (!_unify(n1[idx1], v1, n2.slice(idx2, idx2 + realLength(n2) - realLength(n1)), v2, changes)) {
            return false;
          }
