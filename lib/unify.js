  ...
  boxit = function(elem, varlist) {
    ...
    if (elem instanceof Variable) {
      ...
      ret = map(elem, function(i) {
        if (i instanceof Variable && i.isGreedy) {
          hasGreedyVar = true;
        }
  ...
  _unify = function(n1, v1, n2, v2, changes) {
    ...
    if (n1 instanceof Variable && n2 instanceof Variable) {
      ...
      if (n1.length > n2.length) {
        return _unify(n2, v2, n1, v2, changes);
      }
      if (n1.length < n2.length && !n1.hasGreedyVar) {
        return false;
      }
      idx1 = 0;
      idx2 = 0;
      while (idx2 < n2.length) {
        if (n1[idx1] instanceof Variable && n1[idx1].isGreedy) {
          if (!_unify(n1[idx1], v1, n2.slice(idx2, idx2 + n2.length - n1.length + 1), v2, changes)) {
            return false;
          }
